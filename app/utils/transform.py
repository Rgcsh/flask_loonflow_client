...
from app.utils.exceptions import PPInvalidMapException
...
def float_four(input_float, size=4):
    ...
    Usage:
        ...
        >>> 3.2
    """
    if isinstance(input_float, float64):
        return round(input_float.item(), size)
...
def format_dict_values(ori_dict, value_type):
    """批量格式化字典的value类型

    :param ori_dict: 原始字典
    :param value_type: 需要转换的类型
    """
    for key, value in ori_dict.items():
        ...
        try:
            ori_dict[key] = value_type(value)
        except TypeError:
            print("Trans form Value Failed!")
            continue
    return ori_dict
